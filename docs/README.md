...
**grassistent** (package `grafana-mcp-agent`, plugin id `mcpagent-app`) is an open-source Grafana **app plugin**: an AI chat agent that connects to any HTTP MCP server(s) and is aware of the Grafana page the user is viewing, suggesting model-generated follow-up prompts as the conversation progresses. The model loop runs in the plugin's Go backend against **AWS Bedrock**.
...
| [06-page-context.md](./06-page-context.md) | Async page-context extraction (dashboard API, panel ids, retry + URL tracking), prompt enrichment |
| [07-protocol.md](./07-protocol.md) | Wire contracts: `ChatRequest`, `AgentEvent`, `SuggestionsRequest`, SSE framing |
...
| [13-suggestions.md](./13-suggestions.md) | Model-generated follow-up suggestions: `/suggestions` endpoint, `Agent.Suggest`, custom context, chip UI |
...
- **Model**: AWS Bedrock via **ConverseStream** (token streaming) for chat, plus one non-streaming **Converse** call per settled turn for suggestions. Default model id `us.anthropic.claude-sonnet-4-5-20250929-v1:0`.
- **UI**: docked right-side panel that pushes page content aside (not an overlay); localStorage chat history + custom context; DOM-injected top-bar trigger; model-generated suggestion chips.
