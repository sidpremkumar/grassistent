...
export function ChatPanel({ onClose }: Props) {
  ...
  /* Context-derived questions shown as tappable hint chips, never pre-filled
   * into the input — tapping one fills the composer. These are generated by the
   * backend model from the recent conversation + page context + custom context,
   * so they stay grounded in what the user is actually doing. */
   ...
   * Sends the last few turns, current page context, and the user's custom
   * context. In-flight requests are aborted so only the newest wins. */
  const refreshSuggestions = useCallback(
    (msgs: ChatMessage[], ctx: PageContext) => {
      ...
      /* Nothing to ground a suggestion on yet. */
      if (history.length === 0 && !hasPageContext(ctx)) {
        setSuggestions([]);
        return;
      }
      ...
      void fetchSuggestions(
        { history, pageContext: ctx, customContext: customContext.trim() || undefined },
        controller.signal,
      ).then((next) => {
   ...
   * collapses into one backend call. Runs on load (page context only), after
   * each answer, and when the page or custom context changes. */
  useEffect(() => {
    if (busy) {
      return;
    }
    const handle = window.setTimeout(() => {
      refreshSuggestions(messages, pageContext);
    }, 400);
    return () => {
  ...
  return (
    <div className={styles.root} data-testid="mcpagent-chat">
      ...
      <div className={styles.inputRow}>
        <AnimatePresence initial={false}>
          {suggestions.length > 0 && input.trim() === '' && !busy && !interaction && (
            <motion.div
